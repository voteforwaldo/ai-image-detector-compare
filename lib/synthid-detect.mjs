...
const UNIVERSAL_PHASE_STRONG = 0.55;
...
function applyUniversalSupport(core, universalPhase) {
  if (universalPhase == null) return { ...core, universalPhase: null };
  const merged = { ...core, universalPhase };
  if (universalPhase >= 0.52) {
    const blend = Math.max(core.phaseMatch, core.phaseMatch * 0.75 + universalPhase * 0.25);
    merged.phaseMatch = blend;
    const phaseScore = 1 / (1 + Math.exp(-18 * (blend - 0.52)));
    merged.confidence = Math.min(1, phaseScore);
    merged.isWatermarked = merged.confidence > 0.5;
  }
  return merged;
}
...
function buildApiResult(core, candidates) {
  ...
  const resolutionNote = core.exact
    ? "Анализ при оригинална резолюция (най-надежден)."
    : `Анализ след мащабиране до ${core.profileKey} — по-ниска надежност при споделени/изрязани снимки.`;
  ...
  return {
    ...
    bannerText: bannerDetected ? "Synth ID - Открит" : "",
  };
}
