...
export function explainExifTags(tags) {
  ...
  const photographer = tagValue(tags, "Creator", "Artist", "By-line", "Author");
  ...
  if (photographer) {
    facts.push({ label: "Автор / Creator", value: photographer.slice(0, 160) });
  }
  if (allCred) {
  ...
  const credSample = [claimGen, c2pa, synthIdHint].filter(Boolean).join(" ");
  const hasC2paProvenance =
    Boolean(c2pa || claimGen) &&
    /c2pa|jumbf|content.?credentials|trainedalgorithmic|algorithmic|synthetic|claimgenerator/i.test(
      credSample
    );
