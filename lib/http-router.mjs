...
function normalizePathname(raw, host = "localhost") {
  try {
    const pathname = new URL(raw, `http://${host}`).pathname;
...
function requestPath(req) {
  ...
  for (const raw of candidates) {
    const pathname = normalizePathname(raw, host);
    // Prefer real app paths over Vercel function mount paths like /server
    if (
  ...
  return candidates.length ? normalizePathname(candidates[0], host) : "/";
}
...
export async function handleRequest(req, res) {
  try {
    ...
    if (method === "GET" && (pathname === "/api/auth-status" || pathname === "/api/auth/status")) {
