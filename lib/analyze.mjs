...
export async function analyzeWithGemini(buffer, mimeType, apiKey) {
  ...
  const res = await fetch(`${geminiAnalyzeUrl()}?key=${encodeURIComponent(apiKey)}`, {
    method: "POST",
    ...
    body: JSON.stringify({
      ...
      generationConfig: geminiAnalyzeGenerationConfig(),
    }),
  });
...
export async function analyzeImage(buffer, filename, mimeType, keys) {
  ...
  const { analyzeWithExiftool } = await import("./exiftool.mjs");
  ...
  return {
    ...
    exiftool:
      exifSettled.status === "fulfilled"
        ? { ok: true, ...exifSettled.value }
        : { ok: false, error: exifSettled.reason?.message || String(exifSettled.reason) },
  };
}
