...
/** Match aiornot.com AI Image Detector: primary = ai_generated; deepfake is separate. */
const AIORNOT_URL =
  "https://api.aiornot.com/v2/image/sync?only=ai_generated&only=deepfake";
...
7. **Задължително** прегледай цялата снимка за **видими** водни знаци (лого, текст, overlay, C2PA badge в кадъра). Невидим SynthID **не** се оценява тук.
...
3. **SynthID (Google)** — **невидим** в пикселите; **зрението ти НЕ може да го види**. **ЗАБРАНЕНО** е да пишеш „открит SynthID“, „вероятен SynthID“, „има SynthID watermark“. Отделен спектрален инструмент проверява SynthID. Ти описваш само **видими** елементи.
4. **Digimarc / faint embedded patterns** — едва видими повтарящи се мотиви в небо, стена, равна повърхност (само ако ги виждаш)
5. **Corner/signature stamps** — generator signatures, repeated semi-transparent text, disclosure labels
...
### I. SynthID — забрана за визуални твърдения
SynthID (Google DeepMind) е **невидим watermark в пикселите**. Не може да се види с око и **този forensic преглед не е официален SynthID Detector**.

Правила:
- **Никога** не пиши „открит SynthID“, „има SynthID“, „вероятен SynthID watermark“.
- Ако видиш **Google/Gemini UI или лого** — това е видим бранд, не SynthID. Опиши го като UI/лого.
- Липса на видими следи **не** означава „human“ и **не** означава „няма SynthID“ — просто не коментирай SynthID.
- Други генератори (Midjourney, SD, DALL·E) нямат Google SynthID, но пак могат да са AI.
...
export async function analyzeWithAiornot(buffer, filename, mimeType, apiKey) {
  ...
  const deepfake = data?.report?.deepfake;
  const deepfakeConf = Math.round((deepfake?.confidence ?? 0) * 100);
  const deepfakeDetected = Boolean(deepfake?.is_detected);
  ...
  // Official field per AI or Not docs — do NOT merge deepfake into this verdict.
  // Their website AI Image Detector shows ai_generated.verdict; deepfake is separate.
  const rawVerdict = String(ai.verdict || "").toLowerCase();
  const verdict =
    rawVerdict === "ai" ? "ai" : rawVerdict === "human" ? "human" : "uncertain";

  const parts = [];
  if (verdict === "ai") {
    parts.push(`AI-generated: ИИ (${aiConf}%) — официален verdict от API`);
  } else if (verdict === "human") {
    parts.push(`AI-generated: човек (${humanConf}%) — официален verdict от API`);
  } else {
    parts.push(`AI-generated: неясно/unknown (ИИ ${aiConf}% / човек ${humanConf}%)`);
  }
  if (deepfake) {
    parts.push(
      deepfakeDetected
        ? `Deepfake (отделен модел): открит (${deepfakeConf}%) — не променя AI-generated вердикта`
        : `Deepfake (отделен модел): не е открит (${deepfakeConf}%)`
    );
  }
  ...
  return {
    provider: "aiornot",
    verdict,
    aiGeneratedVerdict: verdict,
    aiPercent: aiConf,
    humanPercent: humanConf,
    deepfakePercent: deepfake ? deepfakeConf : null,
    deepfakeDetected: deepfake ? deepfakeDetected : null,
    summary: parts.join(". ") + ".",
    generators,
    raw: data,
  };
...
async function callGeminiAnalyze(apiKey, model, buffer, mimeType) {
  const inputEst = estimateInputTokens(GEMINI_PROMPT) + 1200;
...
export async function analyzeWithGemini(buffer, mimeType, apiKey) {
  ...
  const models = [requested, ...GEMINI_FALLBACK_MODELS]
    .filter(Boolean)
    .map((m) => (isModelAllowed(m) ? m : DEFAULT_GEMINI_MODEL))
...
/** Drop hallucinated SynthID claims from Gemini vision when the spectral test is negative. */
export function reconcileGeminiWithSynthid(gemini, synthid) {
  if (!gemini?.ok) return gemini;
  const spectralDetected = Boolean(synthid?.ok && synthid.detected);
  const summary = String(gemini.summary || "");
  if (spectralDetected || !/synth\s*id/i.test(summary)) {
    return gemini;
  }
  ...
  const kept = summary
    .split(/(?<=[.!?])\s+/)
    .filter((sentence) => {
      if (!/synth\s*id/i.test(sentence)) return true;
      return /(?:не\s+(?:е\s+)?(?:открит|установен)|няма|липсва|не\s+може|забран)/i.test(sentence);
    })
    .join(" ")
    .trim();
  ...
  const nextSummary =
    kept ||
    "Визуалният forensic анализ не може да потвърди SynthID — вижте отделния SynthID ред в отчета.";
  const inferred = inferVerdictFromForensicText(nextSummary);
  let verdict = gemini.verdict;
  if (verdict === "ai" && inferred.verdict !== "ai") {
    verdict = "uncertain";
  }

  const conf = verdict === "uncertain" ? Math.min(gemini.confidencePercent ?? 50, 60) : gemini.confidencePercent;
  const aiPercent =
    verdict === "ai" ? conf : verdict === "human" ? Math.max(0, 100 - conf) : 50;
  const humanPercent =
    verdict === "human" ? conf : verdict === "ai" ? Math.max(0, 100 - conf) : 50;

  return {
    ...gemini,
    verdict,
    summary: nextSummary.slice(0, 600),
    confidencePercent: conf,
    aiPercent,
    humanPercent,
  };
...
export async function analyzeImage(buffer, filename, mimeType, keys) {
  ...
  return {
    ...
    gemini: reconcileGeminiWithSynthid(geminiRaw, synthid),
...
function applyPlatformLogoOverride(parsed, forensicProse, rawObj = {}) {
  ...
  // Never trust platform_logo enum alone — models hallucinate "gemini" without seeing a mark.
  const sparkle = detectGoogleSparkleWatermark(fullText, rawObj);
  const branding = detectGoogleAiBranding(fullText, rawObj);
  if (!sparkle && !branding) return parsed;
...
function normalizeGeminiParsed(obj, fallbackText) {
  ...
  const sparkleDetected =
    Boolean(rawObj.google_sparkle) ||
    detectGoogleSparkleWatermark([forensicProse, reconciled.summary].filter(Boolean).join("\n"), rawObj);
  ...
  return {
    ...
    google_sparkle: sparkleDetected,
  };
}
