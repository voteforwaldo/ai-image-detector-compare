...
## СТЪПКА 0 — Google/Gemini branding (ПЪРВО, преди всичко друго)
Прегледай **цялото** изображение за:
- лого **Gemini** (spark/star icon), **Google** „G“, Google AI colors/branding
- текст: „Made with Google AI“, „Created with Google AI“, „Google AI“, „Imagen“, „Gemini“
- UI/frame от Google AI/Gemini app, export badge, disclaimer че е генерирано с Google

**Ако видиш КОЕТО И ДА Е от горното:**
→ verdict MUST be "ai", confidence_percent ≥ 95
→ platform_logo в JSON: "google" или "gemini"
→ **ЗАБРАНЕНО:** „липсват watermarks“, „липсват AI артефакти“, „естествена/автентична снимка“, verdict "human"
→ Google/Gemini branding = **воден знак и AI маркер**, не доказателство за реална камера
...
9. **Никога** не твърди „липсват watermarks/AI артефакти“ без да си сканирал ъглите, ръбовете и центъра за лога (особено Google/Gemini).
10. Google/Gemini лого или „Made with Google AI“ **отменя** verdict "human" — винаги "ai".
...
{"verdict":"ai|human|uncertain","confidence_percent":0-100,"platform_logo":"google|gemini|openai|midjourney|other|none","summary":"1–2 изречения на български, echo на ключовото заключение","focus_regions":[{"label":"кратко","note":"какво е подозрително","x":0.0,"y":0.0,"w":0.0,"h":0.0,"severity":"warn|info"}]}

platform_logo: "google"/"gemini" ако видиш Google/Gemini branding; "none" само ако сигурен си че го няма.
...
function detectGoogleAiBranding(text) {
...
function applyPlatformLogoOverride(parsed, forensicProse) {
  const platform = String(parsed.platform_logo || parsed.visible_platform_logo || "")
    .toLowerCase()
    .trim();
  ...
  const hasGooglePlatform =
    googlePlatforms.has(platform) || detectGoogleAiBranding(fullText);
  ...
  if (claimsNoAiWatermarks(summary) || parsed.verdict === "human") {
    summary =
      `Открито Google/Gemini branding (лого или AI маркер) — изображението е AI-генерирано. ` +
  ...
  return {
    ...parsed,
    verdict: "ai",
    confidence_percent: Math.max(parsed.confidence_percent, 95),
