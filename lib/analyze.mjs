...
export async function analyzeWithAiornot(buffer, filename, mimeType, apiKey) {
  ...
  const aiGeneratedVerdict =
    ai.verdict === "ai" ? "ai" : ai.verdict === "human" ? "human" : "uncertain";

  // Сайтът на AI or Not показва и deepfake; силен deepfake сигнал = ИИ/манипулация.
  let verdict = aiGeneratedVerdict;
  if (deepfakeDetected && deepfakeConf >= 50 && aiGeneratedVerdict === "human") {
    verdict = "ai";
  } else if (deepfakeDetected && deepfakeConf >= 50 && aiGeneratedVerdict === "uncertain") {
    verdict = "ai";
  }

  const parts = [];
  if (aiGeneratedVerdict === "ai") {
    parts.push(`AI-generated: ИИ (${aiConf}%)`);
  } else if (aiGeneratedVerdict === "human") {
    parts.push(`AI-generated: човек (${humanConf}%)`);
  } else {
    parts.push(`AI-generated: неясно (ИИ ${aiConf}% / човек ${humanConf}%)`);
  }
  if (deepfake) {
    parts.push(
      deepfakeDetected
        ? `Deepfake: открит (${deepfakeConf}%)`
        : `Deepfake: не е открит (${deepfakeConf}%)`
    );
  }
