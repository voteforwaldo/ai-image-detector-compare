import fs from "fs/promises";
import path from "path";
import os from "os";
import { randomUUID } from "crypto";
import { explainExifTags } from "./exiftool-explain.mjs";
import { analyzeWithExifr } from "./exiftool-exifr.mjs";

const EXIF_TIMEOUT_MS = Number(process.env.EXIFTOOL_TIMEOUT_MS) || 20_000;
const ON_VERCEL = process.env.VERCEL === "1";
const USE_EXIFR = process.env.EXIF_PARSER === "exifr" || ON_VERCEL;

let exiftoolSingleton = null;

function extensionFrom(filename, mimeType) {
  const fromName = path.extname(filename || "");
  if (fromName) return fromName;
  const map = {
    "image/jpeg": ".jpg",
    "image/png": ".png",
    "image/webp": ".webp",
    "image/gif": ".gif",
    "image/heic": ".heic",
    "image/heif": ".heif",
    "image/tiff": ".tiff",
  };
  return map[mimeType] || ".bin";
}

const SKIP_ALL_FIELDS = new Set([
  "SourceFile",
  "Directory",
  "FilePermissions",
  "FileInodeChangeDate",
  "FileAccessDate",
  "FileModifyDate",
  "FileCreateDate",
]);

function formatTagValue(value) {
  if (value == null || value === "") return null;
  if (Array.isArray(value)) return value.filter((v) => v != null && v !== "").map(String).join(", ");
  if (typeof value === "object") {
    try {
      return JSON.stringify(value);
    } catch {
      return String(value);
    }
  }
  return String(value);
}

function allFieldsFromTags(tags) {
  return Object.entries(tags)
    .filter(([key, value]) => !SKIP_ALL_FIELDS.has(key) && formatTagValue(value))
    .sort(([a], [b]) => a.localeCompare(b, "bg"))
    .map(([label, value]) => ({ label, value: formatTagValue(value) }));
}

function buildResult(tags, explained, method = "exiftool-vendored") {
  return {
    provider: "exiftool",
    method,
    ok: true,
    summary: explained.summary,
    highlights: explained.highlights,
    facts: explained.facts,
    tagCount: explained.tagCount,
    hasAiMarkers: explained.hasAiMarkers,
    hasC2paProvenance: explained.hasC2paProvenance,
    allFields: allFieldsFromTags(tags),
    tags: pickTags(tags),
  };
}

function exifFailure(message) {
  return {
    provider: "exiftool",
    ok: false,
    error: message,
    highlights: [],
    allFields: [],
    tagCount: 0,
    hasAiMarkers: false,
    hasC2paProvenance: false,
  };
}

const INTERESTING_TAGS = [
  "FileType",
  "MIMEType",
  "FileSize",
  "ImageWidth",
  "ImageHeight",
  "Megapixels",
  "Make",
  "Model",
  "LensModel",
  "Lens",
  "Software",
  "CreatorTool",
  "ProcessingSoftware",
  "HistorySoftwareAgent",
  "DateTimeOriginal",
  "CreateDate",
  "ModifyDate",
  "GPSPosition",
  "GPSLatitude",
  "GPSLongitude",
  "Artist",
  "Creator",
  "Copyright",
  "ImageDescription",
  "Title",
  "ClaimGenerator",
  "DigitalSourceType",
  "ContentIdentifier",
  "C2PA",
  "JUMBF",
  "PNG:Parameters",
  "Parameters",
  "ProfileDescription",
  "ColorSpace",
];

function pickTags(tags) {
  const out = {};
  for (const key of INTERESTING_TAGS) {
    if (tags[key] != null && tags[key] !== "") out[key] = tags[key];
  }
  for (const [key, value] of Object.entries(tags)) {
    if (out[key] != null) continue;
    if (/c2pa|jumbf|claim|credential|ai|generat|synth|trained/i.test(key)) {
      if (value != null && value !== "") out[key] = value;
    }
  }
  return out;
}

async function getLocalExiftool() {
  if (exiftoolSingleton) return exiftoolSingleton;
  const { exiftool } = await import("exiftool-vendored");
  exiftoolSingleton = exiftool;
  return exiftoolSingleton;
}

async function readWithTimeout(et, filePath) {
  return Promise.race([
    et.read(filePath),
    new Promise((_, reject) => {
      setTimeout(() => reject(new Error("ExifTool timeout")), EXIF_TIMEOUT_MS);
    }),
  ]);
}

async function readWithVendored(tmpPath) {
  const et = await getLocalExiftool();
  const tags = await readWithTimeout(et, tmpPath);
  return buildResult(tags, explainExifTags(tags), "exiftool-vendored");
}

export async function analyzeWithExiftool(buffer, filename, mimeType) {
  if (USE_EXIFR) {
    try {
      return await analyzeWithExifr(buffer, filename, mimeType);
    } catch (err) {
      console.error("exifr error:", err?.message || err);
      return exifFailure(
        `Метаданните не можаха да се прочетат (exifr): ${(err?.message || String(err)).slice(0, 200)}`
      );
    }
  }

  const ext = extensionFrom(filename, mimeType);
  const tmpPath = path.join(os.tmpdir(), `exif-${randomUUID()}${ext}`);

  try {
    await fs.writeFile(tmpPath, buffer);
    return await readWithVendored(tmpPath);
  } catch (err) {
    const msg = err?.message || String(err);
    console.error("ExifTool vendored error:", msg);
    try {
      return await analyzeWithExifr(buffer, filename, mimeType);
    } catch (fallbackErr) {
      return exifFailure(
        `Метаданните не можаха да се прочетат. Локално: npm install && Node 20+. Грешка: ${msg.slice(0, 120)}`
      );
    }
  } finally {
    await fs.unlink(tmpPath).catch(() => {});
  }
}
